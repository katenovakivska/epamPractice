...
namespace EF
{
    class Program
    {
        ...
        {
            string connectionString = @"Data Source=.\SQLEXPRESS;Initial Catalog=EFProductCatalog;Integrated Security=True";
        ...
        {
            var productsWithFixedPrice = productService.GetAllProductsByFixedPrice(price);

            foreach (var product in productsWithFixedPrice)
                Console.WriteLine("ProductId: {0}, ProductName: {1}, Category: {2}, Supplier: {3}, ProductPrice: {4}", product.ProductID, product.ProductName, product.Category.CategoryName, product.Supplier.SupplierName, product.ProductPrice);
        }

        static void PrintProductsBySupplier(IProductService productService, string supplier)
        {
            var productsBySupplier = productService.GetAllProductsBySupplierName(supplier);
        ...
        {
            var products = productService.GetAllProductsWithMaxPrice();
        ...
        {
            var products = productService.GetAllProductsWithMinPrice();
            foreach (var product in products)
                Console.WriteLine("ProductId: {0}, ProductName: {1}, Category: {2}, Supplier: {3}, ProductPrice: {4}", product.ProductID, product.ProductName, product.Category.CategoryName, product.Supplier.SupplierName, product.ProductPrice);
        }
    }
